...
def get_data_meteo_api(
    cities_with_coords: dict, weather_variables: list, start_date: str, end_date: str
) -> pd.DataFrame:
    url = "https://archive-api.open-meteo.com/v1/archive?"
    openmeteo = openmeteo_requests.Client()
    daily_data = {"date": pd.date_range(start=start_date, end=end_date, freq="1D")}
    ...
    for city, coords in cities_with_coords.items():
        ...
        for i in range(len(weather_variables)):
            data = pd.DataFrame(
                {f"{city}_{weather_variables[i]}": daily.Variables(i).ValuesAsNumpy()}
            )
        ...
        .agg(
            {
                ...
                **{col: "max" for col in wind_columns},
            }
        )
        .reset_index()
    )
...
def plot_meteo_temperature_time_series(df) -> plt:
    variables = {
        "temperature": {"metric": "Average", "measure": "°C"},
